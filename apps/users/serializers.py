...
class UserSignUpSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        temp = User.objects.filter(email = validated_data['email'])
        if(temp):
...
class UserSignInSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = User
        fields = ('id','user_name', 'email', 'password', 'token', 'token_expires_at')
...
class UserPsswdSerializer(serializers.ModelSerializer):
    user_name = serializers.CharField(read_only=True)
    email = serializers.EmailField()
    password = serializers.CharField(write_only=True)
    token = serializers.CharField(read_only=True)
    ...
    def create(self, validated_data):
        ...
        # Check the password
        if len(user) > 0 :
            user[0].password = make_password(validated_data['password'])
            # Token
            user[0].token = token_hex(30)
            # Token expires after 7 days
            user[0].token_expires_at = datetime.datetime.now() + datetime.timedelta(days=100)
